...
event zeek_init() &priority=10
	...
	local query = ZeekAgent::Query($ev=Agent_WELProcessCreation::process_creation,
	                                $query="SELECT subject_user_id, subject_user_name, subject_domain_name, subject_logon_id, new_process_id, new_process_name, token_elevation_type, process_id, command_line, target_user_sid, target_user_name, target_logon_id, target_domain_name, parent_process_name, mandatory_label FROM process_creation",
	                                $utype=ZeekAgent::ADD);
	ZeekAgent::subscribe(query);
	}

## zeek_time, source, provider_name, provider_guid, computer_name, event_id, task_id, level, pid, tid, keywords, data